...
# 1) Training your Custom NumPy MLP
print("=" * 50)
...
print("=" * 50)
print("TRAINING MODEL 2: Built-in PyTorch MLP")
print("=" * 50)

pt_mlp = PyTorchMLP(input_size=8, hidden_sizes=[64, 32], output_size=2, dropout_rate=0.2)
...
print("=" * 50)
print("TRAINING MODEL 3: PyTorch TabMixer")
print("=" * 50)

tm_model = TabMixer(F=8, D=32, hidden_dim=64, n_blocks=3, n_classes=2, pooling="mean")
...
print(classification_report(y_test, pt_mlp_preds, target_names=class_names))

print("--- TabMixer Classification Report ---")
print(classification_report(y_test, tm_preds, target_names=class_names))


# RUN AN INFERENCE EXAMPLE ON ROW 0
print("\n" + "=" * 50)
print("Example of Prediction on Row 0")
print("=" * 50)
raw_sample = data.data[0]
real_label = class_names[y[0]]
print(f"Real label: {real_label}\n")

print("-> NumPy MLP Predict:")
mlp.predict_single(raw_sample, scaler, class_names)
...
with torch.no_grad():
    ...
    print(f"Label: {class_names[torch.argmax(pt_mlp_probs).item()]}")
...
with torch.no_grad():
    ...
    print(f"Label: {class_names[torch.argmax(tm_probs).item()]}")
